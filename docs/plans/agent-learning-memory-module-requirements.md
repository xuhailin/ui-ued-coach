...
### 4.3 人脑记忆模型与 Agent 记忆映射

人脑记忆系统是很多 AI 记忆设计的隐性蓝本。它不是一个单一数据库，而是多个并行系统：海马体更像情景记忆入口，新皮层沉淀语义知识，基底节承载程序性技能，杏仁核给强情绪事件加权。Agent 记忆设计也不应该只做一个“万能记忆库”，而应该分系统存储、统一召回。

#### 按时长划分

| 人脑记忆 | 说明 | Agent 对应 |
| --- | --- | --- |
| 感觉记忆 | 亚秒级残留，例如视觉残像 | 通常不需要持久化，只存在于输入处理和多模态感知阶段 |
| 短时 / 工作记忆 | 几秒到一分钟，容量很小，是“当前正在想的东西” | 当前 context window、当前任务状态、工具执行状态 |
| 长时记忆 | 持久、容量大，但需要选择性巩固和遗忘 | 长期用户记忆、项目记忆、关系记忆、反思经验库 |

#### 按能否被有意识提取划分

| 人脑记忆 | 说明 | Agent 对应 |
| --- | --- | --- |
| 陈述性记忆 | 能说出来、能被显式检索 | 事实、偏好、事件、关系、项目规则 |
| 非陈述性记忆 | 很难直接说出，但会影响行为 | persona、表达风格、行为底线、工具使用习惯 |

陈述性记忆里最关键的二分是情景记忆和语义记忆：

| 类型 | 说明 | Agent 设计含义 |
| --- | --- | --- |
| 情景记忆 episodic | 带时间、地点、当事人、感受的具体事件，例如“上周三带儿子去公园，他摔了一跤但没哭” | 对应 episode、时间线、关系中的关键互动片段，适合保留来源和上下文 |
| 语义记忆 semantic | 从多次情景中抽离出来的稳定知识，例如“儿子今年八岁”“儿子不爱吃青椒” | 对应实体、属性、事实边、偏好标签，适合图谱或结构化字段 |

这两者存在单向沉淀关系：语义记忆通常是多个情景反复发生后蒸馏出来的。用户不一定记得是哪一百次确认了“儿子不吃青椒”，但会知道这个事实。对 Agent 来说，episode 是原始情景，图谱抽取出的实体、事实和关系边就是语义沉淀。

非陈述性记忆里最重要的是程序记忆。骑车、打字、熟练使用某个工具都属于“会做但说不清”的技能。对应到 AI 产品，它更接近 persona、表达风格、默认工作方式和行为底线，不应该简单等同于数据库里的一条可查事实。更合适的承载方式是系统提示、策略配置、few-shot、fine-tune 或可审计的行为规则。

#### 关键横切维度

| 维度 | 人脑含义 | Agent 设计含义 |
| --- | --- | --- |
| 前瞻记忆 prospective memory | 记住以后要做某件事，例如“月底交房租”“周末陪孩子” | 对应待办、承诺、提醒、未闭环事件 |
| Zeigarnik 效应 | 未完成事件比已完成事件更容易被记住，一旦闭环就会快速褪去 | 未决事件轨应在未闭环时高权重，闭环后自动降权或归档 |
| 自传体记忆 autobiographical | 关于“我是谁”的连续叙事，混合情景和语义 | 对应 AI 自我状态轨，适合叙事而不是强结构化 |
| 情绪记忆 affective | 情绪给记忆加权，强情绪事件更容易长期保留 | 关系轨应保存带情绪标记的 gist，而不是流水账 |
| 元记忆 / 来源记忆 metamemory / source | 知道自己知道什么、从哪里知道、是否确定 | 每条记忆需要 provenance、confidence、推断标记和冲突处理 |

对于陪伴型产品，同一句事实如果带着强情绪，就应该在巩固时优先固化，在召回时更容易浮现。关系轨真正应该存的不是“3 月 5 日聊了什么”，而是“那次用户很低落，Agent 陪伴了很久”这样的情绪化 gist，并保留必要来源。

#### 三个过程性机制

| 机制 | 说明 | Agent 设计含义 |
| --- | --- | --- |
| 巩固 consolidation | 短时记忆不会自动进入长时记忆，需要重复、重要性和整理过程 | 对应 promotion 机制：高价值记忆才进入长期层 |
| 遗忘 forgetting | 遗忘不是缺陷，而是让系统保持可用的筛选机制 | 对应 decay、demotion、失效时间、降权和主动清理 |
| 重巩固 reconsolidation | 每次提取记忆后，记忆会被短暂解锁并重新写回 | 对应叙事更新与漂移风险：关系叙事需要演进，但不能被多轮压缩递归扭曲 |

重巩固对 Agent 是双刃剑。一方面，叙事记忆反复摘要会产生漂移，旧事实可能被语气和后续解释污染；另一方面，关系叙事本来就应该随着关系演进被重写，这是共同记忆“活着”的标志，而不是损坏。工程上需要同时保留关键锚点、来源和版本，避免只剩一段不断自我改写的摘要。

#### 映射到 xiaoqing 四轨

| 四轨 | 认知学来源 | 推荐承载 | 设计重点 |
| --- | --- | --- | --- |
| 用户轨 | 关于他人的语义 + 情景记忆 | 图谱 / 结构化字段 + 关键 episode | 精确、可编辑、可失效、有来源 |
| 未决事件轨 | 前瞻记忆 + Zeigarnik 效应 | 图 + 闭环触发 + 时间规则 | 未闭环时高权重，闭环后降权或归档 |
| AI 自我状态轨 | 自传体记忆 | 叙事 | 承载“我是谁、最近状态、关系连续性”，不强行图谱化 |
| 关系轨 | 情绪加权的共享情景 gist | 叙事为主，关键时刻用图锚定 | 保存情绪、陪伴、转折和里程碑，而不是保存流水账 |

因此，xiaoqing 的长期记忆更适合采用“分系统存，融合层召回”的架构：用户轨和未决事件轨偏图，AI 自我状态轨和关系轨偏叙事。真正的难点不在四轨各自怎么存，而在融合层如何把四轨命中的内容合成为“xiaoqing 此刻对用户的完整理解”，而不是把四份检索结果机械拼接进 prompt。
